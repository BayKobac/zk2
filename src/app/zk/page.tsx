...
export default function ZKWordPage() {
  ...
  useEffect(() => {
    // 게임 로그와 상태를 가져오는 비동기 함수
    const initializeData = async () => {
      try {
        // 1. input.json 데이터 가져오기
        const inputData = await fetch("/zk/input.json").then((r) => r.json());
        setStoredHash(inputData.storedHash);

        // 2. localStorage에서 게임 로그 가져오기
    ...
    initializeData();
}, [walletAddress]); // walletAddress가 변경될 때마다 실행
